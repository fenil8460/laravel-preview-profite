...
<table id="example" class="display table table-condensed" style="width:100%">
    ...
    <tbody>
        ...
        <tr>
            ...
            <form action="/admin/newsletter/shownewsletter" method="post" id="statusForm{{$data->id}}">
                @csrf
                <!-- If status is 1 an unchecked checkbox -->
                @if ($data->Visible == "hide")
                <td><a href="/admin/newsletter/shownewsletter/{{$data->id}}">
                        <div class="btn-group btn-group-xs">
                            <button type="button" class="btn btn-primary">Show</button>
                        </div>
                    </a>
                    <a href='/admin/newsletter/edit/{{ $data->id }}'>🖉Edit</a>
                    <a href='/admin/newsletter/delete/{{ $data->id }}'>❌Del</a>
                </td>
                <input name="id" type="hidden" value="{{$data->id}}">

                @else
                <td style="border-left-style: unset;border-right-style: unset;"><a
                        href="/admin/newsletter/hidenewsletter/{{$data->id}}">
                        <div class="btn-group btn-group-xs">
